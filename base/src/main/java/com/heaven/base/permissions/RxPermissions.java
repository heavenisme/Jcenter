...
package com.heaven.base.permissions;
...
public class RxPermissions {
        ...
        return new ObservableTransformer<T, Boolean>() {
            ...
            public ObservableSource<Boolean> apply(Observable<T> o) {
                return request(o, permissions)
                        // Transform Observable<SysPermission> to Observable<Boolean>
    ...
    public void request(PermissionListener listener,final String... permissions) {
        Observable<Boolean> observable = Observable.just(TRIGGER).compose(ensure(permissions));
        Disposable subscribe = observable.subscribe(new Consumer<Boolean>() {
            @Override
            public void accept(Boolean granted) throws Exception {
                listener.onGranteCallBack(granted);
            }
        });
    }

   public interface PermissionListener{
        void onGranteCallBack(boolean isGrante);
    }
    ...
    private Observable<Permission> requestImplementation(final String... permissions) {
        ...
        for (String permission : permissions) {
            ...
            if (isGranted(permission)) {
                ...
                // Return a granted SysPermission object.
                list.add(Observable.just(new Permission(permission, true, false)));
                continue;
            }

            if (isRevoked(permission)) {
                // Revoked by a policy, return a denied SysPermission object.
